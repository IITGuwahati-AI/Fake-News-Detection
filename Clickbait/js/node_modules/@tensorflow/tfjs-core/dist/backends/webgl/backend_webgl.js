...
var MathBackendWebGL = /** @class */ (function () {
    ...
    MathBackendWebGL.prototype.fusedBatchMatMul = function (a, b, transposeA, transposeB, bias, activation) {
        ...
        var program = new mulmat_packed_gpu_1.MatMulPackedProgram(a.shape, [batch, outerShapeA, outerShapeB], transposeA, transposeB, hasBias, fusedActivation);
        var output = this.makePackedTensor(program.outputShape, dtype);
        var inputs = [a, b];
        if (bias) {
            inputs.push(bias);
    ...
    MathBackendWebGL.prototype.conv2dByMatMul = function (x, filter, convInfo, bias, activation) {
        ...
        if (batchMatMulWillBeUnpacked || !environment_1.ENV.getBool('WEBGL_LAZILY_UNPACK') ||
            ...
            return this.reshape(this.fusedBatchMatMul(xReshaped_1, filterReshaped_1, transposeA, transposeB, bias, activation), convInfo.outShape);
        }
        ...
        var pointwiseConv = this.fusedBatchMatMul(xReshaped, filterReshaped, transposeA, transposeB, bias, activation);
    ...
    MathBackendWebGL.prototype.conv2dWithIm2Row = function (x, filter, convInfo, bias, activation) {
        ...
        var hasBias = bias != null;
        var fusedActivation = activation ? mapActivationToShaderProgram(activation, true) : null;
        var matmulProgram = new mulmat_packed_gpu_1.MatMulPackedProgram(im2Col.shape, [1, numCols, convInfo.outChannels], transposeA, transposeB, hasBias, fusedActivation);
    ...
    MathBackendWebGL.prototype.fusedConv2d = function (x, filter, convInfo, bias, activation) {
        ...
        var program = new conv_gpu_1.Conv2DProgram(convInfo, hasBias, fusedActivation);
        var inputs = [x, filter];
        if (bias) {
            inputs.push(bias);
        }
